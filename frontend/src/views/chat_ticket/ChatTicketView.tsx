...
function ChatTicketView() {
	...
	const ticket = location.state?.ticket ?? fallbackTicket;
			...
			<div className={`border-b px-4 py-4 md:px-8 ${isDark ? 'bg-[#121212] border-[#2a2a2a]' : 'bg-white border-gray-200'}`}>
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center gap-2">
						...
						<Link
							to="/dashboard"
							className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${isDark
					...
					<div className={`rounded-xl border p-6 ${isDark ? 'bg-[#1a1a1a] border-[#2a2a2a]' : 'bg-white border-gray-200'}`}>
						...
						<div className="space-y-4">
							<div>
								...
								<div
									className={`inline-flex items-center gap-2 rounded-lg px-3 py-1.5 ${isDark ? priorityConfig[ticket.priority].bgDark : priorityConfig[ticket.priority].bg
										}`}
								>
