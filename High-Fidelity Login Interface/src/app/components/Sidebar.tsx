...
export function Sidebar({ currentRole, activePage, onNavigate }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { id: 'analytics', label: 'Analytics', icon: BarChart3, roles: [UserRole.ADMIN, UserRole.AGENT] },
  ...
  return (
    <aside className={`w-64 border-r h-screen flex flex-col ${
      ...
      <div className={`p-4 border-t ${isDark ? 'border-[#2a2a2a]' : 'border-gray-200'}`}>
        ...
        <button className={`w-full flex items-center gap-2 px-4 py-2 text-sm rounded-lg transition-colors ${
          isDark 
            ? 'text-gray-400 hover:bg-[#1a1a1a]'
            : 'text-gray-700 hover:bg-gray-50'
        }`}>
